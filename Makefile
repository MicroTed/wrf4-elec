...
physics :
	...
	if [ $(WRF_CHEM) -eq 0 ] ; then \
		if [ $(WRF_ELEC) -eq 0 ] ; then \
		  ( cd phys ; $(MAKE) submodules ; $(MAKE) CF2=" " EF=" " N2M="$(NSSL2M)" ) ; \
		else \
		  ( cd phys ; $(MAKE) submodules ; $(MAKE) EF="$(ELEC_FILES)"  N2M="$(NSSL2ME)" ) ; \
		fi ; \
		...
		if [ $(WRF_ELEC) -eq 0 ] ; then \
		  ( cd phys ; $(MAKE) submodules ; $(MAKE) CF2="$(CHEM_FILES2)" EF=" "  N2M="$(NSSL2M)") ; \
		else \
		  ( cd phys ; $(MAKE) submodules ; $(MAKE) CF2="$(CHEM_FILES2)"  EF="$(ELEC_FILES)"  N2M="$(NSSL2ME)" ) ; \
		fi \
	fi
